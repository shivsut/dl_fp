...
class IceHockeyLearner(gymnasium.Env):
    def __init__(self, args, expert='jurgen_agent', logging_level=None, print_episode_result=False):
        learner = IceHockeyLearner1 if not args.otherSide else IceHockeyLearner2
        self.learner = learner(args, expert, logging_level, print_episode_result)
        x_max = 40
        y_max = 66
        pi = np.pi
        if args.md:
    ...
    def step(self, action):
        return self.learner.step(action)

    def step_async(self, action):
        self.learner.async_res= self.step(action)

    def step_wait(self):
        return self.learner.async_res

    def reset(self, seed=1, options=None):
        return self.learner.reset(seed, options)

    def close(self):
        self.learner.close()

class IceHockeyLearner1(gymnasium.Env):
    def __init__(self, args, expert='jurgen_agent', logging_level=None, print_episode_result=False):
        self.num_envs =1
        self.do_init = True
        self.args = args
        ...
        super(IceHockeyLearner1, self).__init__()
        if args.md:
            self.discrete = discretization(aceel_div=args.md)
            self.action_space = spaces.MultiDiscrete(nvec=self.discrete.K_n, start=self.discrete.start)
        else:
    ...
    def step(self, action):
        ...
        # print(f"team1_actions: {team1_actions}, team2_actions: {team2_actions}")
...
class IceHockeyLearner2(gymnasium.Env):
    def __init__(self, args, expert='jurgen_agent', logging_level=None, print_episode_result=False):
        self.num_envs =1
        self.do_init = True
        self.args = args
        ...
        # pi = 3.2
        self.extract_state_train = extract_featuresV2 if expert=='jurgen_agent' else extract_features
        super(IceHockeyLearner2, self).__init__()
        if args.md:
            self.discrete = discretization(aceel_div=args.md)
            self.action_space = spaces.MultiDiscrete(nvec=self.discrete.K_n, start=self.discrete.start)
        else:
        ...
        self.num_players = 1
        self.team_id = 1
        self.opponent_team_id = 0
        if self.args.use_opponent:
            self.team1 = AIRunner() if self.args.opponent == 'ai' else TeamRunner(args.opponent)
        self.team2 = DummyTeam(self.num_players, self.team_id )
    ...
    def step(self, action):
        ...
        soccer_state = to_native(self.state.soccer)
        logging.info('calling agent')
        team1_actions = self.team1.act(team2_state, team1_state, soccer_state) if self.args.use_opponent else []
        team2_actions = self.team2.act(action)
        print(f"team1_actions: {team1_actions}, team2_actions: {team2_actions}")
        ...
        if self.recorder:
            self.recorder(team1_state, team2_state, actions=team1_actions, soccer_state=soccer_state)
        ...
        team2_state_next = [to_native(p) for p in self.state.players[1:2]]
        soccer_state = to_native(self.state.soccer)
        opponent_state = team1_state_next[0] if self.args.use_opponent else team1_state_next
        p_features = np.array(self.extract_state_train(team2_state_next[0], opponent_state, soccer_state, 0).flatten().tolist())
        if np.isnan(p_features).any():
            self.terminated = True
    ...
    def reset(self, seed=1, options=None):
        ...
        # print(f"opponent_state: {opponent_state} and team2_state_next: {team2_state_next}" )
        p_features = self.extract_state_train(team2_state_next[0], opponent_state, soccer_state, 0).flatten().tolist()
        return np.array(p_features), {'terminal_observation': np.array(p_features)}
